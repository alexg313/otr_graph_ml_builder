...
class ObjectTree:
    ...
    def read(self, object_map, image, image_rgb):
        '''
        ...
        # check if image is greyscale
        assert image_rgb.ndim == 3, 'Check if the read image is color.'
    ...
    def connect(self, plot=False, export_df=False):
        ...
        '''
        df, img, img_rgb = self.df, self.img, self.img_rgb
...
class Graph:
    ...
    def _get_text_features(self, data):
        ...
        # [[n_lower, n_upper, n_spaces,n_alpha, n_numeric, n_digits],[coordinates]]
        features = []
    ...
    def _pad_text_features(self, feat_arr):
        ...
        if self.max_nodes > feat_arr.shape[0]:
            ...
            target = feat_arr[:self.max_nodes, :-1]
...
if __name__ == "__main__":
    print(os.getcwd())
    df = pd.read_csv(
        '/Users/ap/Documents/OTR/git/Graph-Convolution-on-Structured-Documents/output.csv')
    img = cv2.imread(
        '/Users/ap/Documents/OTR/git/Graph-Convolution-on-Structured-Documents/wa_ticket_gr.jpg', 0)
    img_rgb = cv2.imread(
        '/Users/ap/Documents/OTR/git/Graph-Convolution-on-Structured-Documents/wa_filled_1.jpeg')
    print(img_rgb.shape)
    tree = ObjectTree()
    tree.read(df, img, img_rgb)
    ...
    print(f"Adj Matrix A: {A}")
    print('-----------------------------------------------------------------\n')
    print(f"List of text entities: {X}")
